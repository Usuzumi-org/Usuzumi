...
Use `.uzu-panel-nav` with `data-uzu-panel-nav` for side navigation that switches `.uzu-panel` sections. Use `.uzu-code-block` for copyable code snippets: external highlighters can write token spans into the existing `code` element, keep `data-uzu-code-source` as the plain copy value, and set `--uzu-code-block-bg` / `--uzu-code-block-fg` on the block when their theme requires different colors. Use `.uzu-prose[data-uzu-markdown]` for the built-in Markdown subset. The Markdown renderer intentionally supports only headings, paragraphs, unordered lists, links, inline code, and fenced code blocks; full Markdown documents should still be generated by a dedicated documentation pipeline.
