...
| `--uzu-topbar-leading-gap` | `8px` | topbar leading slot gap | local topbar |
...
`.uzu-sidebar-layout` creates a reusable sidebar-plus-main grid for documentation and application pages. It keeps the sidebar column at `--uzu-sidebar-layout-sidebar-width`, uses a 28px default column gap through `--uzu-sidebar-layout-gap`, and limits a child `.uzu-sidebar.uzu-scroll-area` so long navigation does not push the active content far below the first viewport. Add `data-uzu-sidebar-layout` and a `.uzu-sidebar-layout-toggle[data-uzu-sidebar-toggle]` button when the layout should collapse; the button can sit in `.uzu-topbar-leading` before the brand and point back with `data-uzu-sidebar-target`. `data-uzu-sidebar-default="auto"` starts wide screens expanded and narrow screens collapsed, while narrow screens open as a top dropdown by default. Use `data-uzu-sidebar-mobile="inline"` to keep the older inline narrow layout. `data-uzu-sidebar-collapse-on-select="narrow"` closes the sidebar after mobile-style navigation selection. The runtime animates open/close, delays `hidden` until close animation ends, writes `data-uzu-sidebar-collapsed`, keeps `aria-expanded` / `aria-controls` synchronized, and emits `uzu-sidebar-layout-change`. `.uzu-sidebar-layout-controls` remains available as a compact inline control row when the toggle should sit inside the layout. `.uzu-scroll-area` can also bound local preview or specimen regions with `--uzu-scroll-area-max-height` while preserving the public scrollbar styling. `.uzu-section-centered` centers its section head, large body copy, and direct `.uzu-flex` action row. `.uzu-grid-4` extends `.uzu-grid` to four equal columns and collapses to one column on narrow screens. `.uzu-break-anywhere` is a small utility for long tokens such as CSS variables, package names, and URLs inside constrained tables or cards.
...
- Add `data-uzu-sidebar-layout` to a `.uzu-sidebar-layout` and point a `.uzu-sidebar-layout-toggle[data-uzu-sidebar-toggle]` button at it with `data-uzu-sidebar-target="#layout-id"` when documentation or app navigation should collapse. Put the toggle in `.uzu-topbar-leading` before the brand for page-level navigation, or use `.uzu-sidebar-layout-controls` inside the layout for a local inline control row. `data-uzu-sidebar-default` accepts `auto`, `expanded`, or `collapsed`; `data-uzu-sidebar-mobile` accepts `dropdown` or `inline` and defaults to `dropdown` on narrow screens; `data-uzu-sidebar-collapse-on-select` accepts `false`, `narrow`, or `always`.
