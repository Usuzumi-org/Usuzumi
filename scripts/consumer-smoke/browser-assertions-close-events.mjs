...
if (!value.themeMediaReinitListenerRegistered || value.themeAfterReinitPreferenceChange !== 'dark' || !value.themeMediaReinitListenerRemoved) throw new Error(`Browser consumer destroy()/init() should restore and clean up the theme media listener: ${JSON.stringify({ registered: value.themeMediaReinitListenerRegistered, theme: value.themeAfterReinitPreferenceChange, removed: value.themeMediaReinitListenerRemoved })}`);
