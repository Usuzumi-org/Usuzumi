...
if (value.calloutSignatureWhiteSpace !== 'nowrap' || value.calloutSignatureOverflowWrap !== 'normal' || value.calloutSignatureWordBreak !== 'keep-all') throw new Error(`Browser consumer signature text should stay on one line: ${JSON.stringify({ whiteSpace: value.calloutSignatureWhiteSpace, overflowWrap: value.calloutSignatureOverflowWrap, wordBreak: value.calloutSignatureWordBreak })}`);
...
if (value.topbarOverflowWideState.directLinks !== 4 || value.topbarOverflowWideState.menuItems !== 0 || !value.topbarOverflowWideState.menuHidden || value.topbarOverflowWideState.overflow > 1) throw new Error(`Browser consumer topbar overflow should keep all links visible when width allows: ${JSON.stringify(value.topbarOverflowWideState)}`);
if ((value.topbarOverflowWideState.directCurrentText || value.topbarOverflowWideState.menuCurrentText) !== 'Home' || (value.topbarOverflowMediumState.directCurrentText || value.topbarOverflowMediumState.menuCurrentText) !== 'Home' || (value.topbarOverflowNarrowState.directCurrentText || value.topbarOverflowNarrowState.menuCurrentText) !== 'Home') throw new Error(`Browser consumer topbar overflow should preserve current item state while resizing: ${JSON.stringify({ wide: value.topbarOverflowWideState, medium: value.topbarOverflowMediumState, narrow: value.topbarOverflowNarrowState })}`);
if (value.topbarOverflowMediumState.directLinks < 1 || value.topbarOverflowMediumState.directLinks >= 4 || value.topbarOverflowMediumState.menuItems < 1 || value.topbarOverflowMediumState.menuHidden || value.topbarOverflowMediumState.overflow > 1) throw new Error(`Browser consumer topbar overflow should move trailing links into the menu at medium width: ${JSON.stringify(value.topbarOverflowMediumState)}`);
if (value.topbarOverflowNarrowState.directLinks > value.topbarOverflowMediumState.directLinks || value.topbarOverflowNarrowState.menuItems < value.topbarOverflowMediumState.menuItems || value.topbarOverflowNarrowState.menuHidden || value.topbarOverflowNarrowState.overflow > 1 || Math.abs(value.topbarOverflowNarrowState.leadingCenter - value.topbarOverflowNarrowState.actionsCenter) > 1) throw new Error(`Browser consumer topbar overflow should keep brand and actions on one row at narrow width: ${JSON.stringify(value.topbarOverflowNarrowState)}`);
if (!value.topbarOverflowMenuOpenState.open || value.topbarOverflowMenuOpenState.expanded !== 'true' || value.topbarOverflowMenuOpenState.firstItemRole !== 'menuitem' || value.topbarOverflowMenuOpenState.firstItemTabindex !== '0') throw new Error(`Browser consumer topbar overflow menu did not open as a menu: ${JSON.stringify(value.topbarOverflowMenuOpenState)}`);
if (!value.topbarOverflowMenuClosedAfterSelect) throw new Error('Browser consumer topbar overflow menu did not close after selecting an item');
if (value.topbarOverflowRestoredState.directLinks !== 4 || value.topbarOverflowRestoredState.menuItems !== 0 || !value.topbarOverflowRestoredState.menuHidden) throw new Error(`Browser consumer topbar overflow should restore links when width returns: ${JSON.stringify(value.topbarOverflowRestoredState)}`);
if ((value.topbarOverflowDynamicCurrentState.directCurrentText || value.topbarOverflowDynamicCurrentState.menuCurrentText) !== 'Components') throw new Error(`Browser consumer topbar overflow should preserve current state changed after initialization: ${JSON.stringify(value.topbarOverflowDynamicCurrentState)}`);
if (value.topbarOverflowDestroyState.directLinks !== 4 || value.topbarOverflowDestroyState.menuItems !== 0 || !value.topbarOverflowDestroyState.menuHidden) throw new Error(`Browser consumer topbar overflow destroy() should restore the nav: ${JSON.stringify(value.topbarOverflowDestroyState)}`);
if ((value.topbarSpyHashState.directCurrentText || value.topbarSpyHashState.menuCurrentText) !== 'Three' || value.topbarSpyHashState.currentAria !== 'location') throw new Error(`Browser consumer topbar hash spy did not mark the matching hash target: ${JSON.stringify(value.topbarSpyHashState)}`);
if (value.topbarSpyMissingState.firstAria !== 'page' || value.topbarSpyMissingState.missingAria !== '' || (value.topbarSpyMissingState.directCurrentText || value.topbarSpyMissingState.menuCurrentText) !== 'One') throw new Error(`Browser consumer topbar hash spy should keep the original current item when a hash target is missing: ${JSON.stringify(value.topbarSpyMissingState)}`);
if (value.topbarSpyDestroyState.firstAria !== 'page' || value.topbarSpyDestroyState.thirdAria !== '' || value.topbarSpyDestroyState.menuItems !== 0) throw new Error(`Browser consumer topbar hash spy destroy() should restore original state: ${JSON.stringify(value.topbarSpyDestroyState)}`);
